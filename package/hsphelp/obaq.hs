...
y(0.0) : 内部座標Y(実数)
%inst
OBAQの内部座標をqdraw命令で表示される画面上の座標に変換します。
(x,y)に内部座標を指定すると、var_x,var_yに画面上の座標が代入されます。
...
^p
	モード        	内容
------------------------------------------------------------------------
	colsw_none     出現時コリジョンチェック無し
	colsw_active   出現時active状態の岩との接触を避ける
	colsw_all      出現時reserve状態以外の岩との接触を避ける
^p
