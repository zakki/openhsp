...
; Easy3D for HSP3
;
;ヘルプのhs形式への移殖作業者：GENKI
;2007/05/10 :v3.0.1.5のhs公開
;2007/05/10 :v3.0.1.9のhs公開
;2007/06/25 :v3.0.2.0のhs公開
...
%index
E3DChkConflict2
２つの形状データの指定したパーツ同士が衝突しているかどうかを判定。
%group
E3D:当たり判定
%prm
p1, p2, p3, p4, p5, p6
...
%inst
２つの形状データの指定したパーツ同士が、衝突しているかどうを、判定します。
...
alphaval が、１．０の時は、不透明に、
alphavalが、０．０の時は、完全に透明になります。

alphavalに、１．０以外の値を指定した場合は、
...
%index
E3DDirToTheOther
hsid1で識別される形状を、hsid2で識別される形状の方向を、向くようにする。
%group
E3D:モデル向き
%prm
p1, p2
...
次の表は一般的なPC用のキーボードで対応しているキーとの対応表です。
ただし、一般的にノートPCのテンキーは非常に押しにくいので、テンキーはあまり多用しないほうがいいでしょう。
html{
<table border="1">
...
p9: [OUT] 変数　：　retz
%inst
(vecx1, vecy1, vecz1)と、（vecx2, vecy2, vecz2）の両方に垂直で、大きさが１ なベクトル(retx, rety, retz)を取得する。
...
E3DSigLoad, E3DAddMotionで作成したオブジェクトを破棄します。
%group
E3D:後処理
%prm
p1
...
p17: [OUT] 変数　：　nz
%inst
befposとnewposを結ぶ線分と、groundhsidで識別される地面との、あたり判定をします。
...
%index
E3DSetVertPos
パーツ内の頂点番号の頂点の座標（ローカル座標）をセットします。
%group
E3D:モデル情報
%prm
p1, p2, p3, p4, p5, p6
...
%index
E3DSetBillboardPos
billboardidで識別されるビルボードの位置を指定します。
%group
E3D:ビルボード
%prm
p1, p2, p3, p4
...
%index
E3DSetBillboardOnGround
ビルボードの位置を、指定したＸＺ座標の地面の高さにセットします。
%group
E3D:ビルボード
%prm
p1, p2, p3, p4, p5, p6
...
%index
E3DGetInvisibleFlag
モデルのパーツのInvisibleFlag(見えないフラグ)を取得します。
%group
E3D:モデル情報
%prm
p1, p2, p3
...
%index
E3DSetInvisibleFlag
モデルのパーツのInvisibleFlag(見えないフラグ)をセットします。
%group
E3D:モデル情報
%prm
p1, p2, p3
...
%index
E3DChkConfWall
作成した壁データと、キャラクターのあたり判定（壁スリ）を行います。
%group
E3D:当たり判定
%prm
p1, p2, p3, p4, p5, p6, p7, p8, p9, p10
...
p10: [OUT] 変数　：　nz
%inst
E3DSetMovableArea, または、E3DLoadMQOFileAsMovableAreaで、作成した、壁データと、キャラクターのあたり判定を行います。
...
MQOファイルで作成した壁のデータを、移動可能領域情報として読み込みます。
%group
E3D:壁
%prm
p1, p2, p3
...
３Dサウンド再生時に関係するリスナーのパラメータを設定します。
%group
E3D:サウンド
%prm
p1, p2
...
%index
E3DFillUpNaviLine
ナビポイントとナビポイントの間を、指定した分割数で補間します。
%group
E3D:ナビライン
%prm
p1, p2, p3
...
ナビラインのデータを元に、キャラクターの位置と向きを自動的に設定する。
%group
E3D:ナビライン
%prm
...
%index
E3DSetQAxisAndDeg
クォータニオンを指定した軸に関して、指定した角度だけ回転した姿勢をセットします。
%group
E3D:算術
%prm
p1, p2, p3, p4, p5
...
mpinfo.MPI_SCALEXは、Ｘ方向の拡大情報
mpinfo.MPI_SCALEYは、Ｙ方向の拡大情報
mpinfo.MPI_SCALEZは、Ｚ方向の拡大情報
...
mpinfoflag.MPI_QUAが１の場合は、
mpinfo.MPI_QUAのクォータニオン情報を
モーションポイントにセットします。

mpinfoflag.MPI_TRAXが１の場合は、
...
%index
E3DCopyMotionFrame
モーションのフレーム番号の全てのモーションポイントの情報をコピーします。
%group
E3D:モーション
%prm
p1, p2, p3, p4, p5
...
%index
E3DMultQVec
ベクトルに対して、クォータニオンによる回転をした後のベクトルを出力する。
%group
E3D:算術
%prm
p1, p2, p3, p4, p5, p6, p7
...
p7: [OUT] 変数　：　aftvecz
%inst
(befvecx, befvecy, befvecz)というベクトルに対して、qidで識別されるクォータニオンによる回転をした後のベクトルを、(aftvecx, aftvecy, aftvecz)に代入します。
...
E3DInitのmultisamplenumに０以外を指定した場合は、この命令は使えません。
（エラーになります。）
...
%index
E3DPickVert
画面上の２Ｄ座標を指定して、その座標に一番近い頂点を検出する。
%group
E3D:モデル情報
%prm
p1, p2, p3, p4, p5, p6, p7, p8, p9, p10
...
%index
E3DGetOrgVertNo
最適化後の頂点番号に対応する、rok,mqoファイル内での頂点の番号を取得します。
%group
E3D:モデル情報
%prm
p1, p2, p3, p4, p5, p6
...
%index
E3DLoadIM2File
モデルデータに、im2ファイルのボーン影響情報をセットします。
%group
E3D:ボーン影響
%prm
p1, p2
...
%index
E3DSaveIM2File
モデルデータの影響マップ情報を、im2ファイルに書き出します。
%group
E3D:ボーン影響出力
%prm
p1, p2
...
%index
E3DSigImport
形状データ(hsid)に、形状データ(filename)をインポートします。
%group
E3D:形状データ
%prm
p1, p2, p3, p4, p5, p6, p7, p8, p9, p10
...
%index
E3DRdtscStart
プロセッサのタイムスタンプカウンタがどれくらい増えたかを取得できます。
%group
E3D:同期
%prm
...
p15: [OUT] 数値または、変数　：　resx
p16: [OUT] 数値または、変数　：　resy
p17: [OUT] 数値または、変数　：　resz
p18: [OUT] 数値または、変数　：　lastparent
%inst
ＩＫによるジョイントの回転を行います。
目標の座標などを指定すると、
そこに近づくように、回転します。
...
%index
E3DGetChildJoint
指定したジョイントの、子供ジョイントの数と子供のシリアル番号を取得します。
%group
E3D:モデル情報
%prm
p1, p2, p3, p4, p5
...
%index
E3DSetUVTile
タイル画像を敷き詰めたテクスチャのＵＶ座標をタイル番号でセット。
%group
E3D:モデル情報
%prm
p1, p2, p3, p4, p5, p6
...
地面データ(hsid)に、地面データ(filename)をインポートします。
%group
E3D:地面
%prm
p1, p2, p3, p4, p5, p6, p7, p8, p9, p10
...
%index
E3DChkThreadWorking
作成したスレッドが、現在、動作中であるかどうかをチェックする。
%group
E3D:スレッド管理
%prm
p1, p2, p3, p4
...
%index
E3DGetSpecularPower
任意のパーツの任意の頂点のspecular powerの100倍の値を取得します。
%group
E3D:モデル情報
%prm
p1, p2, p3, p4
...
モーションの最終フレームで、ジャンプする先のモーションとそのフレーム番号を指定。
%group
E3D:モーション
%prm
p1, p2, p3, p4
...
この補間モーションのもーしょんIDはver3001では常に０です。
