...
^
指定した座標にあるドットの色データが選択色として設定されます。(color命令で指定したのと同じ状態になります)
...
サイズの指定が省略されるか0以下の場合は、objsizeで設定されている値が使用されます。
...
詳しくは、プログラミングマニュアルのCEL関連命令についてを参照してください。
...
%inst
指定したウィンドウIDが持つ画像素材の分割数を設定します。
画像素材の分割数は、celput命令により画像を描画する際に参照されます。
たとえば、256×256ドットの画像を2×4に分割する設定にした場合、
celput命令により128×64ドットの画像8枚分として扱うことができるようになります。
^
p1で、画像素材を持つウインドゥIDを指定します。
p2,p3で、横(X)・縦(Y)方向の分割数を指定することができます。
分割数は１以上の整数値となります。規定外の値を指定した場合は、エラーとなります。
^
画像素材の分割数は、ウィンドウIDごとに保存されており、cls命令などで初期化された
時点では、1×1(分割なし)の設定になっています。
詳しくは、プログラミングマニュアルのCEL関連命令についてを参照してください。
...
ただし、描画位置は、現在のカレントポジション(pos命令で指定された値)が中心となります。
また、描画される画像のサイズは、画像素材となるウィンドウIDのサイズ及び分割数で決定されます。
^
