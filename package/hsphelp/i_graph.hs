...
%inst
グラフィック面をウィンドウに描画する基点座標、及びスケールを設定します。
...
^
ウィンドウ描画スケールは、グラフィック面をウィンドウに描画する際のスケール(倍率)を設定します。
これにより、描画されたグラフィック面を拡大したり、縮小する形でウインドウに表示させることができます。
ウィンドウ描画スケールに(1,1)を設定すると、通常の状態(グラフィック面をそのままウインドウに表示)となります。
描画スケールに(2,2)を設定すると、縦横それぞれ2倍に拡大された状態でウインドウに表示されます。
描画スケールは、必ずウインドウ全体に収まる範囲にする必要があります。(ウインドウサイズよりも小さく縮小することはできません)
ウインドウ上に表示される画像はスケーリングされますが、マウスの座標はウインドウ上の実座標となることに注意してください。
また、標準ランタイム上では、ウィンドウ描画スケールは、配置オブジェクトに対しては反映されません。
