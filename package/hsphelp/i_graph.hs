...
^p
  画面モード    マクロ名             内容
-----------------------------------------------------------------------------------
   0                                 フルカラーモード(24bitカラー)で初期化する
   1                                 パレットモード(1670万色中256色)で初期化する(Windows版のみ)
   32           screen_offscreen     描画先として初期化する(HGIMG4のみ)
   64           screen_usergcopy     描画用シェーダーを割り当てる(HGIMG4のみ)
^p
Windows版のHSP3では、パレットモード(1670万色中256色)で初期化を行うことが可能です。
仮想画面はHSP3Dishランタイム上で使用する際には、描画先として使用できません(画像を読み込むために使用されます。)。
また、HGIMG4ランタイム使用時は、screen_offscreenとscreen_usergcopyを指定することが可能です。詳しくは、HGIMG4プログラミングガイドを参照してください。
^
