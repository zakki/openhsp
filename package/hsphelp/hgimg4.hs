...
また、板ノード(gpplateで生成)との衝突は正しく判定されないことがあるため、gppraytestを使用する際には厚みのある物体(箱など)を使用するようにしてください。
%href
gppcontact
getcoli
getwork
...
^p
	...
	GPNODEINFO_MATERIAL     ノードに設定されたマテリアル名(*)
...
GPNODEINFO_MATERIALを指定した場合は、ノードに設定されたマテリアル名を取得します。メッシュモデルではないノードやggploadで読み込まれていない3Dノードを指定した場合は、空の文字列("")が返されます。
