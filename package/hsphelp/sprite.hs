...
------------------------------------------------------------
           ...
           3    ESMAP_PRM_NOTICE   通知アイテムの検査(未実装)
           4    ESMAP_PRM_HITOFSX  スプライトとBGマップ接触判定時のXオフセット値
           5    ESMAP_PRM_HITOFSY  スプライトとBGマップ接触判定時のYオフセット値
           6    ESMAP_PRM_HITSIZEX スプライトとBGマップ接触判定時のXサイズ値
           7    ESMAP_PRM_HITSIZEY スプライトとBGマップ接触判定時のYサイズ値
...
アトリビュート値は、0から65535(16bit)までの値を設定することができます。この情報は、es_bghit命令によるBGとの当たり判定に使用されます。
...
--------------------------------------------------
  ...
  0x1000  ESMAP_ATTR_MAX    キャラクタNo.の最大値
^p
...
アトリビュート値は、BGマップが使用するキャラクターNo.ごとに設定される補助的な情報です。0から65535までの値が設定されており、es_bghit命令によるBGとの当たり判定に使用されます。
...
es_bghit命令は、任意の矩形を移動させた場合にBGマップのキャラクタに接触するかどうかを判定し、結果のリストを作成します。任意の座標をもとに当たり判定を行う場合は、es_bghitpos命令を使用してください。
結果は、es_getbghit命令により取得することが可能です。
p1パラメーターでBGNo.を指定します。BGNo.は0～15までを指定可能です。
p2パラメーターでスプライトNo.を指定します。スプライトの座標は、BGマップ上の座標として扱われます。
(p3,p4)パラメーターで、移動させるX,Y方向の移動量をドット単位で指定します。
...
%inst
新規に登録されるスプライトのマップ参照設定を行います。
この命令により、スクロールするマップと座標を同期させるスプライトを手軽に登録することが可能になります。
(es_setparent命令により、マップとリンクされた状態と同様になります)
...
------------------------------------------------------------------
  ...
  0x1000  ESSPMAPHIT_GETEVENT  マップのイベント情報を取得する
  0x2000  ESSPMAPHIT_EVENTWIPE マップのイベント情報取得時にアトリビュートを消去する
