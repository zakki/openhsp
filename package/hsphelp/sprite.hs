...
------------------------------------------------------------
           ...
           4    ESMAP_PRM_WIPECHR  スプライト消去時に発生させるデコレーションID
...
アトリビュート値は、BGマップが使用するキャラクターNo.ごとに設定される補助的な情報です。0から65535までの値が設定されており、es_bghit、es_bghitpos命令によるBGとの当たり判定に使用されます。
...
また、es_get命令で取得できるスプライトパラメーター「ESI_MOVERES」に結果の概要が格納されます。この値のbit0～7(0～255)には自身と重なる領域のマップアトリビュート値(bit0～7)が格納されています。
...
何らかの理由で設定に失敗した時は、システム変数statにマイナス値が返ります。
%href
es_bgmap
es_putbg
es_bgattr
