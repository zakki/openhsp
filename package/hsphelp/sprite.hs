...
情報の種類は、p3のinfo numberで指定します。[]内はヘッダファイルで定義されている定数です。
すべての情報は32bitの整数で保存されています。スプライトの座標や、加速度、倍率などは固定小数値となっているため実際の値が、65536倍されているので注意してください。
^p
info number :
	  0 = flag値 [ESI_FLAG]
	  1 = スプライトのX座標 [ESI_POSX]
	  2 = スプライトのY座標 [ESI_POSY]
	  3 = 1フレームに移動する距離X [ESI_SPDX]
	  4 = 1フレームに移動する距離Y [ESI_SPDY]
	  5 = プログレスカウント値(未使用) [ESI_PRGCOUNT]
	  6 = アニメーションカウント値 [ESI_ANIMECOUNT]
	  7 = キャラクタNo. [ESI_CHRNO]
	  8 = type値 [ESI_TYPE]
	  9 = 自由落下加速度X [ESI_ACCELX]
	 10 = 自由落下加速度Y [ESI_ACCELY]
	 11 = 弾みやすさ [ESI_BOUNCEPOW]
	 12 = バウンドフラグ [ESI_BOUNCEFLAG]
	 13 = オプション値 [ESI_OPTION]
	 14 = 優先順位 [ESI_PRIORITY]
	 15 = 特殊効果(EP値) [ESI_ALPHA]
	 16 = αフェード用パラメーター [ESI_FADEPRM]
	 17 = X方向倍率 [ESI_ZOOMX]
	 18 = Y方向倍率 [ESI_ZOOMY]
	 19 = 回転角度 [ESI_ROTZ]
^p
...
情報の種類は、p2のinfo numberで指定します。[]内はヘッダファイルで定義されている定数です。
...
^
p3パラメーターにより、内部処理を保留することができます。通常は、命令を実行すると、描画されたスプライトは移動を行いますが、 p3に1を指定すること描画のみを行うようにすることができます。 
p3に2を指定することでアニメーションを停止させることができます。
p3に4を指定することでes_setgosub命令で設定したコールバックを無効にすることができます。
...
%inst
スプライト用のキャラクタパターンとして登録されているデータを、指定した座標に表示します。この命令は、スプライトとは関係なく画面にキャラクタを転送する命令です。
