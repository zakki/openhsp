...
%inst
MOC情報に角度情報を設定します。オフセット番号0から3つのパラメータが対象になります。
整数値(256で１回転)をラジアン単位に変換してパラメーターを書き込みます。
角度を指定するパラメーター以外では正常な値にならないので注意してください。
%href
objset3
...
p2でコリジョン判定時に、指定された値に対するスケール(倍率)を実数で指定することができます。
setcolscale命令によってコリジョンスケールが設定されなかった場合は、モデルごとの標準コリジョンスケールが使用されます。
...
モード値に、$10000～$70000までの値を指定することで、目的の座標(または角度)として指定したIDのオブジェクトが持つ値を参照することができます。
