...
またその時点での画面初期化モード(ウィンドウモードかフルスクリーンモードか)によって、
HGIMG3の描画モードも合わせて設定されます。
...
%inst
シーンの情報の初期化を行ないます。
すべてのモデル、オブジェクト、テクスチャ、イベントリストの情報は破棄され、
まったく新しいシーンの状態に戻します。
%href
hgini
hgbye
...
p1で、コピー元のテクスチャIDを指定、(p2,p3)でコピーされる元の画像
にあたる座標を指定します。(gcopy命令と同様です)
コピー先は、現在の描画先に指定されているウィンドウIDで、
...
mode値が-1または省略された場合は、通常の登録処理が行なわれ、
mode値にすでに登録されたテクスチャIDを指定すると、
同じIDのテクスチャを現在のウィンドウ内容で更新します。
^
通常は、texload命令、texload2命令によってファイルから
テクスチャを読み込むようにしてください。
%href
texload
texload2
...
ビデオカードがテクスチャの色モードやサイズに対応していない場合には、
ファイルが存在する場合でもエラーになることがあるので注意してください。
...
"filename" : 登録するテクスチャの画像ファイル
%inst
p1で指定したモデルに対してトゥーンシェーディングのモードを設定し、
色の情報を持つテクスチャファイルを読み込みます。
^
あらかじめ、addxfile命令によりX形式の3Dモデルが読み込まれている必要があります。
トゥーンシェーディングを設定する場合には、あらかじめトゥーンテクスチャと呼ばれる
色情報を持った画像データを作成しておかなければなりません。
...
保存されたトゥーンテクスチャは、loadtoon命令で読み込んで使用することが可能なほか、
画像を編集することにより、より細かいトゥーンシェーディングの表現を行なうことが可能になります。
...
texmes命令は、テクスチャに対して文字列を描画するだけなので、
実際に文字列を表示するためには、テクスチャをhgrotate命令やスプライトなどで表示する必要があります。
...
texidが省略された場合は、次に登録されるテクスチャが参照されます。
(sx,sy)で板のX,Yサイズを設定します。省略された場合は、setsizefで指定された値が適用されます。
addplate命令とほぼ同じですが、光源計算を行なうことと、テクスチャUV座標の指定ができない点が
異なっています。
...
エフェクトモデルは頂点ごとに色が設定され、光源計算を行なわない
特別なモデルで、形状をseteprim命令でカスタマイズすることができます。
...
^
こうしてできたイベントは、setevent命令によっていつでも
オブジェクトに対して適用することができます。
...
Xオフセットのデフォルト値は、0.5が設定されており、この場合はX方向の半分にあたる位置を中心にして、
...
これは、テクスチャーアニメーションを実現させるためのもので、
イベントが実行されると、テクスチャのUV座標が(p2,p3)で指定された値(ドット数)だけ加算されます。
...
オブジェクト削除イベントは、現在イベントを実行しているオブジェクト
そのものを削除する命令です。
%href
event_regobj
newevent
...
オブジェクト生成イベントは、regobj命令と同様にイベント実行時に
新しいオブジェクトを生成します。
パラメーターとして、modelにモデルIDを、eventにイベントIDを指定する
ことができます。また、生成されるオブジェクトの座標は、イベントを
実行しているオブジェクトの座標が継承されます。
...
modelに、addeprim命令によって作成されたモデルIDを指定します。
モデルIDがマイナス値の場合は、イベントが設定されているオブジェクトに
割り当てられているモデルが対象になります。
paramにseteprim命令と同様のパラメーターIDを指定します。
ただし、色指定(パラメーターID16以降)については、A→R→G→Bの順番で
別なパラメーターIDが割り当てられています。
...
p3(0) : 情報ID
%inst
p2で指定されたオブジェクトのアニメーションに関する情報を取得して、
p1で指定された変数に代入します。
...
アニメーションループフラグは、アニメーション開始時は0ですが、
最後まで再生された場合には1が返されます。
...
最初にモデル全体と最初のアニメーションを含んだデータをaddxfile命令により読み込み、
さらに追加でアニメーションだけを登録する場合にaddxanim命令を使用します。
必ず、モデルやボーンの構成は同一のものから出力されている必要があります。

複数のアニメーションは、objact命令によって切り替えることが可能です。
切り替えるためのアニメーションインデックス値は、最初にaddxfile命令で読み込まれたアニメーションを0として、
以降addxanim命令で追加するたびに1,2,3…と増加していきます。

追加できるアニメーションの最大数は、標準で16(もとのxファイル含む)です。
最大数を拡張する場合は、setreq命令によってアニメーション最大数(SYSREQ_MDLANIM)
を適切に設定してください。
...
modelspeed命令によって標準のアニメーション再生速度が設定されなかった場合は、
15(ms)がオブジェクト初期化時に設定されます。
...
p1,p2,p3,p4
p1(0) : モデルID
p2(1.0) : コリジョン判定時のスケールX(実数値)
p3(1.0) : コリジョン判定時のスケールY(実数値)
p4(1.0) : コリジョン判定時のスケールZ(実数値)
%inst
p1で指定されたモデルIDが持つ標準のコリジョンスケールを設定します。
p2でコリジョン判定時に、指定された値に対するスケール(倍率)を実数で指定することができます。
以降は、指定されたモデルを使用するオブジェクト初期化時のコリジョンスケールとして設定されます。
modelcols命令によって標準のコリジョンスケールが設定されなかった場合は、1.0(等倍)が設定されます。
オブジェクトごとのコリジョンスケールは、setcolscale命令によって設定することも可能です。
...
p4(1.0) : コリジョン判定時のスケールZ(実数値)
%inst
p1で指定されたオブジェクトIDが持つコリジョンスケールを設定します。
p2でコリジョン判定時に、指定された値に対するスケール(倍率)を実数で指定することができます。
setcolscale命令によってコリジョンスケールが設定されなかった場合は、
モデルごとの標準コリジョンスケールが使用されます。
...
ChildObjIDで指定されたオブジェクトは、ObjIDで指定されたオブジェクトと
親子関係を持つことになります。
以降は親となる、ObjIDで指定されたオブジェクトの座標、回転角度、スケールを
継承した形で表示が行なわれます。
また、親のオブジェクトが削除された場合には、その子供すべても同時に
削除されます。
子供を持っている親オブジェクトを、ChildObjIDに指定することで多階層の
構造を設定することが可能です。
すでに何らかの親を持っているオブジェクトを子供に指定することはできません。
ChildObjIDにマイナス値を指定すると、ObjIDで指定したオブジェクトが持つ
親子設定をすべて破棄します。
%href
regobj
delobj
...
プロジェクションモードが0の場合は、通常のオブジェクトとしてカメラの
NearZ値、FarZ値に従ってZ方向のクリッピングが行なわれます。
プロジェクションモードが0の場合は、特殊なオブジェクトとしてカメラの
FarZ値は無視されます。
背景など、一部のオブジェクトだけを遠くに描画するような場合に、
プロジェクションモードを変更することができます。
%href
regobj
...
これ以降は、dmm～で始まる命令を使用して、DirectSoundによる
サウンド再生機能を使用することが可能になります。
...
p2で設定する音量を指定します。0が最大の音量(0db)となり、
-10000が最小の音量(無音)になります。
...
ストリーミング再生時は、p2でループポイントの指定を行なうことが
可能です。ループポイントは、サンプル単位(44.1kHzの場合は、44100分の1秒)で
数値を指定します。
...
ファイルは、memfile命令で指定したメモリ内のイメージを含め、dpmファイルなど
HSPで使用可能なファイルをすべて利用することができます。

p1に登録するサウンドID番号(0から始まる任意の数値)を指定することで、
これ以降はサウンドID番号をもとに各種機能を使用することができるようになります。
p1の指定を省略、またはマイナス値にした場合は、登録されていない
サウンドID番号を自動的に割り当てます。
...
p2が省略されるか0の場合は、サウンドデータの最初から再生され、
それ以外の場合は最初からのオフセット位置として途中から再生されます。
...
デフォルトでは、目的の座標(または角度)はオブジェクトのworkグループ情報に設定された値が使用されます。
あらかじめ、setwork命令により目標の座標(または角度)を設定しておく必要があります。

Mode 0 : 目標値に向けて移動(または回転)

目的の座標(または角度)に向けて移動(または回転)します。
(ターゲットモードが0の場合は移動、1の場合は回転角度の変更になります。)
オプションパラメーターX,Y,Zにより、１フレームあたりの最大移動量を設定することができます。
たとえば、(1,1,1)が指定されている場合は、X,Y,Zそれぞれ１フレームあたり最大1づつしか
移動を行ないません。
...
これは、オブジェクトのモードでOBJ_MOVEを指定している場合に、指定された座標に向けて
移動させる場合などに使用するためのものです。
このモードは、常にターゲットモード0(座標)を指定するようにしてください。

Mode 3、 4 : 目標値に向けて角度を設定

目的の座標に向けて角度を設定します。
さらに、目的の座標に向うためのベクトルを、dirグループ情報に設定します。
これは、オブジェクトのモードでOBJ_MOVEを指定している場合に、指定された座標に向けて移動させる場合などに使用するためのものです。
オプションパラメーターX,Y,Zにより、もともとのモデルが向いているベクトルを指定しておく必要があります。
このモードは、常にターゲットモード0(座標)を指定するようにしてください。

Mode 5、 6 : 目標値に向けて角度を設定(ディレイ)

モード3,4と同様に、目的の座標に向けて角度を設定します。
ただし、一度に目的の方向を向くのではなく、一定の範囲までしか角度を移動させません。
角度の許容範囲は、オブジェクトのworkグループ情報に設定されたx,y,z値が使用されます。
つまり、workグループのxに0.1が指定されていた場合は、最大でも1フレームあたり0.1の単位でしか目的の方向を向きません。それ以外の動作は、モード3,4と同じです。
このモードは、常にターゲットモード0(座標)を指定するようにしてください。

モード値に、$10000～$70000までの値を指定することで、目的の座標(または角度)として
指定したIDのオブジェクトが持つ値を参照することができます。
たとえば、$10000+5を指定した場合は、目的の座標としてオブジェクトID5の値を使用します。
さらに、$10000～$70000の値がモード0～6に対応して、この場合はモード0として処理されます。
...
(ターゲットモードは常に0が使用されます。)
また、モード3、モード5にはそれぞれ一定の動作回数制限が設けられており、
一定時間の間動作する機能として利用することができます。
...
%inst
リアルタイムタイマーの値を設定します。
リアルタイムタイマーは、1ms単位の高精度な時間を計測するもので、
HGIMG3の動作中に使用可能です。
...
取得タイプが0の場合は、1msごとにカウントアップされるリアルタイムタイマーの値を
そのまま取得します。
...
拡張画面制御命令
%prm
var,modelid,mode,rate
var        : メッシュ情報が代入されている変数名
modelid(0) : マップ設定を行なうモデルID
mode(0)    : 設定モード
rate(0.01) : 設定レート(実数値)
%inst
指定したモデルにメッシュ情報を設定します。
addmesh命令によって作成されたメッシュモデルにマップ情報を設定します。
メッシュ情報は、メッシュの頂点ごとに、任意の値を2次元配列変数として格納されています。
varで指定された変数には、あらかじめメッシュ情報を格納する2次元配列の値が設定されている必要があります。
設定モードにより、以下の機能を選択することが可能です。
^p
モード  内容
-----------------------------------------------
0       Y座標を設定(Y=メッシュ情報×rate)
1       X座標を設定(X=メッシュ情報×rate)
2       Z座標を設定(Z=メッシュ情報×rate)
3       U座標を設定(U=メッシュ情報×rate)
4       V座標を設定(V=メッシュ情報×rate)
+16     法線の再設定時にフラットシェーディングを適用
$100    メッシュ情報をマップデータとしてUVを設定
^p
...
mode(0) : 作成モード
%inst
画像データをもとにメッシュ情報を作成し、変数に代入します。
メッシュ情報は、メッシュの頂点ごとに、任意の値を2次元配列変数として格納するものです。
varで指定される変数には、あらかじめdim命令で2次元配列の宣言をしておく必要があります。
(2次元配列は、「dim 変数,X分割数+1,Y分割数+1」を確保する必要があるので注意してください。)
設定モードにより、以下の機能を選択することが可能です。
^p
モード  内容
-----------------------------------------------
...
1       白以外のピクセルに1を設定する
2       白以外のピクセルに2を設定する
17      白以外のピクセルに1を、白の場合は0を設定する
18      白以外のピクセルに2を、白の場合は0を設定する
^p
