...
・hspswプラグインについて
	...
	hspswプラグインの詳細は、付属のマニュアル(docs/hspsw.txt)、サンプル(samples/hspsw)を
	参照してください。
...
・英語版スクリプトエディタの同梱について
	...
	HSPインストールフォルダに、英語版スクリプトエディタが同梱されています。
...
・古いプラグイン・ランタイムの同梱終了について
	...
	hsp3jsなど代替の機能があるため、フルセット版への同梱は終了させて頂きますのでご了承ください。
	以下のプラグイン・ランタイムは、HSP3.6での同梱を行いません。

		・HGIMG/HGIMGXプラグイン
		・llmod3モジュール
		・HSP3MTランタイム
		・HSP3MTライブラリ(hspsdk/hsplib)

	これらは長い期間更新がなく、古いシステムに依存しているため新しく使用することは推奨できません。
	アップデートされたHSPから使用できなくなるわけではありませんが、フルセット版への同梱は
	終了させて頂きますのでご了承ください。

	HSP3MTランタイム及びライブラリは、古いシステム向けに作成されたもので、
	hsp3utfやhsp3_64などの新しいランタイムでも代用が可能です。
	また、OpenHSPとしてソースが公開されているため、ライブラリそのものの配布も
	必要がなくなったと判断しています。
