...
(4). パラメータ起動について
   ...
   パラメータをつけて起動した場合、最初に表示されるAndroid/iOS選択ウィンドウをスキップすることができます。

   Androidの場合（例）:	-a -p"c:\android\project"

   iOSの場合（例）:		-i -p"c:\ios\project"



(5). 履歴

   2022/04/08 (Ver0.5) : パラメータ付きで起動された場合は、Android/iOS選択画面をスキップするように修正。

   2022/04/07 (Ver0.4) : 「戻る」ボタンを追加。

   2022/03/28 (Ver0.3) : コマンドラインからプロジェクトフォルダのテキストボックスに直接パスをセットするように修正。(Androidの場合、-a -p"c:\test") (iOSの場合 -i -p"c:\test")
