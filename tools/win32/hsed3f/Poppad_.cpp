	 ...
	 switch(iMsg) {
		case WM_COMMAND :
			 switch (LOWORD (wParam)){
                ...
                case IDM_HSPMAN2 :
					wsprintf( helpopt,"%s\\hsphelp\\hspref.htm", szExeDir );
					ShellExecute( NULL, NULL, helpopt, NULL, NULL, SW_SHOW );
					//fileexe("start","hspref.htm");
					return 0 ;

                case IDM_HSPMAN3 :
					wsprintf( helpopt,"%s\\hsphelp\\indexf.htm", szExeDir );
					ShellExecute( NULL, NULL, helpopt, NULL, NULL, SW_SHOW );
			...
			switch (LOWORD (wParam)){
				...
				case IDC_STATICLINK:
					if(HIWORD(wParam) == STN_CLICKED){
						ShellExecute(NULL, NULL, "http://www.onionsoft.net/hsp/", NULL, NULL, SW_SHOW);
						return TRUE;
					}
					return  TRUE;
			}
...
BOOL CALLBACK ConfigBehaviorPageProc (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message){
		...
		case PM_APPLY:
			bUseIni = IsDlgButtonChecked(hDlg, IDC_RADIO2);
			bAutoIndent = IsDlgButtonChecked(hDlg, IDC_CHECK1);
