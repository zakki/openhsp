...
/*
	rev 44
	sjisÇÃëSäpîªíËÇ∆îªífÇµÇƒèCê≥ÇQÅB(naznyark)
*/
	unsigned char a1;
	char fext[1024];
	char finf[1024];
	...
	if (fext[0]!=42) {
		if (extinfo[0]==0) sprintf( finf,"%sÃß≤Ÿ",fext );
					  else strcpy( finf,extinfo );
		sprintf( szFilter, "%s (*.%s)@*.%s@", finf, fext, fext );
	}
	strcat( szFilter,"Ç∑Ç◊ÇƒÇÃÃß≤Ÿ (*.*)@*.*@@" );
...
/*
	rev 43
	mingw : warning : DWORDå^ÇÃñﬂÇËílÇ…-1Çï‘ÇµÇƒÇ¢ÇÈ
	Ç…ëŒèà
*/
